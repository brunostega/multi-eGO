...
def check_inputs(top_df_input, top_df_output, args):
    ...
    if N_mol_input != N_mol_output:
        raise ValueError(
            f"Number of molecules in input and output topology are different: input {N_mol_input}, output {N_mol_output}"
        )
    for i, mol in enumerate(top_df_input["name"]):
        if top_df_input["name"][i] != top_df_output["name"][i]:
            raise ValueError(
                f"Molecules in input and output topology do not correspond: input {top_df_input['name'][i]}, output {top_df_output['name'][i]}"
            )
        H_indices_in = np.char.startswith(top_df_input["atoms_name"].to_numpy()[i], "H")
        H_indices_out = np.char.startswith(top_df_output["atoms_name"].to_numpy()[i], "H")
        if np.any(
            atom_element(top_df_input["atoms_name"].to_numpy()[i][~H_indices_in])
            != atom_element(top_df_output["atoms_name"].to_numpy()[i][~H_indices_out])
        ):
            raise ValueError(f"Found difference in atoms of molecule {mol}")

    if N_mol_input < np.maximum(MOL_I + 1, MOL_J + 1):
        raise ValueError(
            f"Matrix indedes {MOL_I} {MOL_J} are not compatible with the given topology: Not enough molecules in topology"
        )
    if len(top_df_input["atoms_name"].to_numpy()[MOL_I]) * len(top_df_input["atoms_name"].to_numpy()[MOL_J]) != len(mat["ai"]):
        raise ValueError(
            f'Number of elements in input matrix is not compatible with the number of atoms in topology: matrix : {len(mat["ai"])} , topology mol_{MOL_I+1} {len(top_df_input["atoms_name"].to_numpy()[MOL_I])} - mol_{MOL_J +1 } {len(top_df_input["atoms_name"].to_numpy()[MOL_J])}'
        )
...
This tools modifies an input matrix to change the protonation state accordingly to the input and output topology
    ...
    parser.add_argument(
        "--output_top", type=str, required=True, default=None, help="Output topology with the correct protonation state"
    )
...
mat_noH = mat.loc[
    (~(mat["ai_name"].str.startswith(args.H_name)) | ((mat["ai_name"] == args.H_name)))
    & ((~(mat["aj_name"].str.startswith(args.H_name))) | ((mat["aj_name"] == args.H_name)))
]
...
where_H = (~(mat_out["ai_name"].str.startswith(args.H_name)) | ((mat_out["ai_name"] == args.H_name))) & (
    (~(mat_out["aj_name"].str.startswith(args.H_name))) | ((mat_out["aj_name"] == args.H_name))
)
