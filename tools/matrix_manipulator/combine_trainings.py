...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="If true checks overlap of matrices domains. default true, but memory expensive",
    )
    parser.add_argument(
        "--check_element",
        type=bool,
        default=False,
        action=argparse.BooleanOptionalAction,
        help="If true checks element differences betweeen domain top and global top. default true",
    )
        ...
        data = [
            {
                "system": system_name,
                "range": details["range"],
                "path_top": details["path_top"],
                "path_mat": details["path_mat"],
            }
...
for i, intra in enumerate(paths_intramats):
    ...
    if topol_domains[i]["N_tot_atoms"][index_mi - 1] * topol_domains[i]["N_tot_atoms"][index_mj - 1] != len(intra_appo[0]):
        raise ValueError(
            f"""The number of atoms of molecule {index_mi} times molecule {index_mj} is different from that of matrix read
{topol_domains[i]['N_tot_atoms'][index_mi-1]*topol_domains[i]['N_tot_atoms'][index_mj-1]} {len(intra_appo[0])}"""
    ...
    if float(end + 1 - start) != n_atoms_after:
        print(
            f"    Number of atoms in full system is not compatible with number of atoms in matrix after terminal removal: {end -start} vs {n_atoms_after}"
        )
        exit()
...
is different from the one obtaied from the reference topology {dim_matrix}. Check one of the two before continuing."""
    ...
    print(
        f"    -FULL LENGTH MATRIX PASSED: {global_matrix_path} will be used as the basic matrix. On top will be added the single domains"
    )
...
for i in range(len(ranges)):
    ...
    if i > 0 and args.check_overlap:
        new_indeces = [
            "{}_{}".format(x, y)
            for x, y in zip(
                final_intramat[1][np.where(domain_mask_linear)[0]], final_intramat[3][np.where(domain_mask_linear)[0]]
            )
        ]
        ...
        if intersect.size > 0:
            print(
                f"****Found an overlap between ranges {ranges[i]} and range {ranges[i-1]}. \n****The contacts will be merged to consider the one with shortest distance or the one with information different from 0"
            )
            # if new intramat probability is 0 use old information
            TEMP_INDEX = 4
            intramats[i][TEMP_INDEX][index_inters_a] = np.where(
                intramats[i][5][index_inters_a] == 0,
                intramats[i - 1][TEMP_INDEX][index_inters_b],
                intramats[i][TEMP_INDEX][index_inters_a],
            )
            # if are both greater than 0 use the one which has the shortest distance (4 column)
            intramats[i][TEMP_INDEX][index_inters_a] = np.where(
                np.logical_and(
                    np.logical_and(intramats[i][5][index_inters_a] != 0, intramats[i - 1][5][index_inters_b] != 0),
                    intramats[i - 1][4][index_inters_b] < intramats[i][4][index_inters_a],
                ),
                intramats[i - 1][TEMP_INDEX][index_inters_b],
                intramats[i][TEMP_INDEX][index_inters_a],
            )
            ...
            if np.any(intramats[i][TEMP_INDEX][index_inters_a] != intramats[i - 1][TEMP_INDEX][index_inters_b]):
                ...
                print(f"{i-1}")
                print(
                    intramats[i - 1][1][index_inters_b][
                        np.where(intramats[i][TEMP_INDEX][index_inters_a] != intramats[i - 1][TEMP_INDEX][index_inters_b])[0]
                    ]
                )
                print(
                    intramats[i - 1][3][index_inters_b][
                        np.where(intramats[i][TEMP_INDEX][index_inters_a] != intramats[i - 1][TEMP_INDEX][index_inters_b])[0]
                    ]
                )
                print(
                ...
                print(f"\n{i}")
                print(
                    intramats[i][1][index_inters_a][
                        np.where(intramats[i][TEMP_INDEX][index_inters_a] != intramats[i - 1][TEMP_INDEX][index_inters_b])[0]
                    ]
                )
                print(
                    intramats[i][3][index_inters_a][
                        np.where(intramats[i][TEMP_INDEX][index_inters_a] != intramats[i - 1][TEMP_INDEX][index_inters_b])[0]
                    ]
                )
                print(
    ...
    print(
        f"    -Added {paths_intramats[0].split('/')[-1].split('_')[0]} in ranges {ranges[i]} in {time.time()-start_time:.2f} s"
    )
...
if index_mi == index_mj and index_mi == 1:
    ...
    np.savetxt(
        f"frankestein_inter_{index_mi}_{index_mj}.ndx",
        final_intramat.T,
        delimiter=" ",
